...
import { team as staticTeam } from "@/lib/content";
...
export function Team({ copy, points, imageSrc }: TeamProps) {
  const fallback = img(staticTeam.image);
  const activeCopy = copy || staticTeam.copy;
  const activePoints = points?.length ? points : staticTeam.points;
  const activeImage = imageSrc || fallback.src;
  const imageAlt = fallback.alt;
      ...
      <Container>
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
          <Reveal className="order-2 lg:order-1">
            <Eyebrow>{staticTeam.eyebrow}</Eyebrow>
